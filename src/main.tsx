...
function App(): React.JSX.Element {
  ...
  const savePreset = async (global: boolean): Promise<void> => { if (!activeModel || !activeProject) return; const name = window.prompt(global ? "Global preset name" : "Project preset name", "Studio render")?.trim(); if (!name) return; try { await invoke("save_preset", { projectId: activeProject.id, name, settings: activeModel.settings, global }); setToast({ tone: "notice", text: `${global ? "Global" : "Project"} preset saved.` }); } catch (error) { setToast({ tone: "error", text: String(error) }); } };
